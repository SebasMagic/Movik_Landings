...
Replace both photographic backgrounds (hero + mid-page "photo break") with a custom SVG/CSS motif: 3–4 soft, curved lines suggesting a route/highway, rendered in brand purple at varying opacity (roughly matching the existing glow orbs' intensity), drifting slowly and continuously sideways (30–40s linear/ease loop — slow enough to read as ambient, not attention-grabbing).
...
- Applied identically (same motif, adjusted scale) in the hero and in the "photo break" section between Pain and Solution, so the two dark/atmospheric moments in the page feel like one consistent visual language instead of two different treatments.
...
- **Photo break**: same route-line motif as hero (in place of the Unsplash road photo), same centered quote overlay.
- **Solution/Rates, Comparison table, How it works, Final CTA, Footer**: keep v3 as-is.

### 4. Deliverable

A new file, `Landing Page v4.dc.html`, in `Movik UI Kit y brand assets (1)/`, based on v3, with the hero and photo-break backgrounds swapped to the route-line motif described above. Verified by opening the file in a browser before hand-off.
